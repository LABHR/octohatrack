...
def main(): 
  ...
  if args.generate_html:
    ...
    if not args.show_contributors and len(non_code_contributors) == 0:
      print("No non-code contributors for %s. No HTML output being generated.")
      sys.exit(0)

    html_file = "%s_contrib.html" % repo_name.replace("/", "_")
    f = open(html_file, "w")

    f.write("<style>.contrib{display:inline-block; padding: 10px;} .gen {padding-top: 40px}</style>")
    f.write("<h1>Non-code contributions for %s</h1>" % repo_name)

    if args.limit > 0:
      f.write("(for the most recent %d issues/pull requests)<br><br>" % args.limit)
    ...
    f.write("<div class='gen'>Generated by <a href='https://github.com/glasnt/octohat'>octohat</a></div>")
